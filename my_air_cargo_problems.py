...
class AirCargoProblem(Problem):
    ...
    def get_actions(self):
        ...
        # DoneTODO create concrete Action objects based on the domain action schema for: Load, Unload, and Fly
    ...
    def h_ignore_preconditions(self, node: Node):
        ...
        executed.
        '''
        count = len([s for i, s in enumerate(self.state_map) if s in self.goal and node.state[i] is 'F'])
        return count
...
def air_cargo_p2() -> AirCargoProblem:
    ...
    neg = [expr("At({}, {})".format(cp, a)) for cp in cargos + planes for a in airports
           if expr("At({}, {})".format(cp, a)) not in pos]
